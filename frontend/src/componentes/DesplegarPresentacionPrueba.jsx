// src/componentes/DesplegarPresentacionPrueba.jsx
import React, { useEffect, useState } from 'react';
import servidor from '../config';

function DesplegarPresentacionPrueba() {
  const [presentaciones, setPresentaciones] = useState([]);
  const [chat, setChat] = useState([]);
  const [comando, setComando] = useState('');
  
  // Usamos un ID estático para el usuario, ya que no se hace la autenticación.
  const usuarioId = 7438201161;  // ID de usuario estático (también es el chat_id).
...
export default DesplegarPresentacionPrueba;
